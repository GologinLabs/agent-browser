...
function parseFindLocator(positional: string[]): {
  ...
  if (mode === "role") {
    ...
    if (!role || !action || !ACTIONS.has(action)) {
      throw new AppError("BAD_REQUEST", "Usage: gologin-local-agent-browser find role <role> <action> [value] [--name <name>]", 400);
    }
  ...
  if (mode === "text") {
    ...
    if (!text || !action || !ACTIONS.has(action)) {
      throw new AppError("BAD_REQUEST", "Usage: gologin-local-agent-browser find text <text> <action> [value]", 400);
    }
  ...
  if (mode === "label") {
    ...
    if (!label || !action || !ACTIONS.has(action)) {
      throw new AppError("BAD_REQUEST", "Usage: gologin-local-agent-browser find label <label> <action> [value]", 400);
    }
  ...
  if (mode === "placeholder") {
    ...
    if (!placeholder || !action || !ACTIONS.has(action)) {
      throw new AppError("BAD_REQUEST", "Usage: gologin-local-agent-browser find placeholder <placeholder> <action> [value]", 400);
    }
  ...
  if (mode === "first" || mode === "last") {
    ...
    if (!selector || !action || !ACTIONS.has(action)) {
      throw new AppError("BAD_REQUEST", `Usage: gologin-local-agent-browser find ${mode} <selector> <action> [value]`, 400);
    }
  ...
  if (mode === "nth") {
    ...
    if (!nth || !selector || !action || !ACTIONS.has(action) || Number.isNaN(Number(nth))) {
      throw new AppError("BAD_REQUEST", "Usage: gologin-local-agent-browser find nth <n> <selector> <action> [value]", 400);
    }
  ...
  throw new AppError(
    "BAD_REQUEST",
    "Usage: gologin-local-agent-browser find <role|text|label|placeholder|first|last|nth> ...",
    400
  );
}
