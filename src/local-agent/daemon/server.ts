...
function matchProfileRoute(pathname: string): string | undefined {
  ...
  try {
    ...
    if (method === "GET" && pathname === "/profiles") {
      writeJsonResponse(
        response,
        200,
        await profileManager.list({
          source: (url.searchParams.get("source") as "local" | "remote" | "all" | "auto" | null) ?? undefined,
    ...
    if (method === "POST" && pathname === "/profiles") {
      const body = (await readJsonBody(request)) as ProfileCreateRequest;
      writeJsonResponse(response, 200, await profileManager.create(body));
      return;
    }

    if (method === "POST" && pathname === "/profiles/import") {
      const body = (await readJsonBody(request)) as ProfileImportRequest;
      writeJsonResponse(response, 200, await profileManager.import(body));
      return;
    }

    const profileId = matchProfileRoute(pathname);
    if (method === "GET" && profileId) {
      writeJsonResponse(
        ...
        await profileManager.get(
          profileId,
          (url.searchParams.get("source") as "local" | "remote" | "all" | "auto" | null) ?? undefined
        )
      );
      return;
    }
    ...
    if (method === "PATCH" && profileId) {
      const body = (await readJsonBody(request)) as ProfileUpdateRequest;
      writeJsonResponse(response, 200, await profileManager.update(profileId, body));
      return;
    }

    if (method === "DELETE" && profileId) {
      const body = (await readJsonBody(request)) as ProfileDeleteRequest | undefined;
      writeJsonResponse(response, 200, await profileManager.delete(profileId, body?.remote === true));
      return;
    }

    const syncProfileId = pathname.match(/^\/profiles\/([^/]+)\/sync$/)?.[1];
    if (method === "POST" && syncProfileId) {
      writeJsonResponse(response, 200, await profileManager.sync(syncProfileId));
      return;
    }

    const proxyCheckProfileId = pathname.match(/^\/profiles\/([^/]+)\/proxy-check$/)?.[1];
    if (method === "GET" && proxyCheckProfileId) {
      const payload: ProfileProxyCheckResponse = await profileManager.checkProxy(proxyCheckProfileId);
      writeJsonResponse(response, 200, payload);
      return;
    }
