...
import { detectChallengeSignal } from "../lib/sessionSignals";
...
export class SessionManager {
  ...
  private async ensureSessionHealthy(
    ...
    if (session.page.isClosed() || !session.browser.isConnected()) {
      ...
      throw new AppError(
        "SESSION_LOST",
        "browser session closed unexpectedly; the site may have shown a challenge or Orbita may have crashed",
        410,
        details
      );
    }

    if (!options.detectChallenge) {
      return;
    }

    const challenge = await session.page
      .evaluate(() => ({
    ...
    const signal = challenge ? detectChallengeSignal(challenge) : undefined;
    if (!signal) {
      return;
    }
    ...
    throw new AppError(
      "CHALLENGE_DETECTED",
      `page appears blocked by ${signal.reason}; stop the current site flow and inspect the challenge before continuing`,
      409,
      {
        sessionId: session.sessionId,
        profileId: session.profileId,
  ...
  private async findReusableProfileSession(profileId: string): Promise<SessionRecord | undefined> {
    for (const session of Array.from(this.sessions.values())) {
      ...
      try {
        await this.ensureSessionHealthy(session);
        return session;
      } catch (error) {
        if (error instanceof AppError && error.code === "SESSION_LOST") {
          continue;
        }
  ...
  private async getSessionOrThrow(
    ...
    await this.ensureSessionHealthy(session, options);
    return session;
  }
  ...
  async open(request: OpenSessionRequest): Promise<OpenSessionResponse> {
    ...
    if (!profileId) {
      const created = await createManagedProfile(this.config, token, sessionId, request.proxy);
    ...
    if (profileId) {
      const existingForProfile = await this.findReusableProfileSession(profileId);
      if (existingForProfile) {
        if (request.sessionId && request.sessionId !== existingForProfile.sessionId) {
          throw new AppError(
            "PROFILE_BUSY",
            `profile ${profileId} is already open in session ${existingForProfile.sessionId}`,
            409,
            {
              profileId,
              sessionId: existingForProfile.sessionId
            }
          );
        }

        if (request.idleTimeoutMs !== undefined) {
          existingForProfile.idleTimeoutMs = request.idleTimeoutMs;
        }

        existingForProfile.currentUrl = await navigatePage(existingForProfile.page, request.url, this.config.navigationTimeoutMs);
        await this.ensureSessionHealthy(existingForProfile, { detectChallenge: true });
        this.resetSnapshotState(existingForProfile, true);
        this.touchSession(existingForProfile);

        return {
          sessionId: existingForProfile.sessionId,
          profileId: existingForProfile.profileId,
          url: existingForProfile.currentUrl,
          liveViewUrl: existingForProfile.liveViewUrl,
          proxy: existingForProfile.proxy,
          idleTimeoutMs: existingForProfile.idleTimeoutMs
        };
      }
    }

    let connection:
      | Awaited<ReturnType<typeof connectToBrowser>>
      | undefined;
    try {
      connection = await connectToBrowser(this.config, token, profileId, request.headless);
      ...
      const temporarySession: SessionRecord = {
        ...
        gologin: connection.gologin,
        currentUrl,
        hasSnapshot: false,
        staleSnapshot: false,
        proxy: resolvedProxy,
        createdAt,
        lastActivityAt: this.nowIso(),
        idleTimeoutMs: request.idleTimeoutMs
      };
      await this.ensureSessionHealthy(temporarySession, { detectChallenge: true });
      const lastActivityAt = this.nowIso();
      if (!resolvedProxy && profileId) {
        resolvedProxy = await getProfileProxy(this.config, token, profileId).catch(() => undefined);
      }
  ...
  async click(sessionId: string | undefined, target: string): Promise<ClickResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "click");
  }

  async type(sessionId: string | undefined, target: string, text: string): Promise<TypeResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "type", text);
  }

  async fill(sessionId: string | undefined, target: string, text: string): Promise<FillResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "fill", text);
  }

  async hover(sessionId: string | undefined, target: string): Promise<HoverResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "hover");
  }

  async focus(sessionId: string | undefined, target: string): Promise<FocusResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "focus");
  }

  async doubleClick(sessionId: string | undefined, target: string): Promise<DoubleClickResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "dblclick");
  }

  async select(sessionId: string | undefined, target: string, value: string): Promise<SelectResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "select", value);
  }

  async check(sessionId: string | undefined, target: string): Promise<CheckResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "check");
  }

  async uncheck(sessionId: string | undefined, target: string): Promise<UncheckResponse> {
    return this.runTargetAction(await this.getSessionOrThrow(sessionId, { detectChallenge: true }), target, "uncheck");
  }

  async press(sessionId: string | undefined, key: string, target?: string): Promise<PressResponse> {
    const session = await this.getSessionOrThrow(sessionId, { detectChallenge: true });
